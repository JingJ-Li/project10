...
Next, we transform rainfall date from wide form to long form.
```{r}
survey_rainfall_season <- survey_final_gis_clean %>% gather(key=rain_season_key, value=rainfall_season, c("rainfall_dry","rainfall_wet"))
survey_SPI_season <- survey_final_gis_clean %>% gather(key=SPI_season_key, value=SPI_season, c("SPI_dry","SPI_wet"))
...
## t test for income from beef
```{r}
incbeef_dry <-  survey_rainfall_season[survey_rainfall_season$rain_season_key=="rainfall_dry",]$incbeef
incbeef_wet <-  survey_rainfall_season[survey_rainfall_season$rain_season_key=="rainfall_wet",]$incbeef
t.test(incbeef_wet,incbeef_dry,na.rm=TRUE,var.equal = FALSE )
```
...
```{r}
t.test(survey_final_gis_clean$SPI_wet6,survey_final_gis_clean$SPI_dry6,na.rm=TRUE,var.equal = FALSE)
...
```{r}
survey_final_gis_clean$studycode<- factor(survey_final_gis_clean$studycode)
aov_drainage <- aov(drainage_area_km ~ studycode, data=survey_final_gis_clean)

summary(aov_drainage)
TukeyHSD(aov_drainage, conf.level=.95)
```
As p-value of F test is extremely small, we are confident that the drainage means of three regions are different. We further compared drainage means of three regions and found region3 (Rolim) has the highest drainage area, OPO has the smallest.  

### effect of region on income from beef
We then wants to see if there is difference in income from beef at different regions.
```{r}
aov_incbeef <- aov(incbeef ~ studycode, data=survey_final_gis_clean)
summary(aov_incbeef)
...
```{r}
aov_rainfall <- aov(rainfall~ studycode, data=survey_final_gis_clean)
...
```{r}
aov_vechval <- aov(vechval~ studycode, data=survey_final_gis_clean)
summary(aov_vechval)
...
We are going to construct vectors containing relevant variables.
```{r}
income <- c ("incbeef","inchf","inccalf","incpigs","incchicken","incsheepgoat","inchorses","incmules","incdonkey","incotherlive","incbolsaescola","incpension","incbolsafam")
inc_region <- subset(survey_final_gis_clean,select="incbeef")
inc_region <- manova(inc_region~survey_final_gis_clean$studycode, na.rm=TRUE)
```
