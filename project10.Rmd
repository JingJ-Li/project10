...
```
```{r}
describe(survey_final_gis[c('drainage_area_km', 'rainfall', 'rainfall_wet', 'rainfall_dry', 'SPI_wet', 'SPI_dry', 'incbeef', 'milkincome', 'vechval', 'lotprice', 'family')], fast=TRUE)
```
...
## t test for vechval 
```{r}
quartiles_1 <- quantile(survey_final_gis[survey_final_gis$any_adaptation==1,]$vechval, probs=c(.25, .75), na.rm = FALSE)
IQR_1 <- IQR(survey_final_gis[survey_final_gis$any_adaptation==1,]$vechval,na.rm = FALSE)
 
Lower_1 <- quartiles_1[1] - 1.5*IQR_1
Upper_1 <- quartiles_1[2] + 1.5*IQR_1
 
data_no_outlier_1<- subset(survey_final_gis[survey_final_gis$any_adaptation==1,]$vechval, survey_final_gis[survey_final_gis$any_adaptation==1,]$vechval> Lower_1 & survey_final_gis[survey_final_gis$any_adaptation==1,]$vechval< Upper_1)

quartiles_0 <- quantile(survey_final_gis[survey_final_gis$any_adaptation==0,]$vechval, probs=c(.25, .75), na.rm = FALSE)
IQR_0 <- IQR(survey_final_gis[survey_final_gis$any_adaptation==0,]$vechval,na.rm = FALSE)
 
Lower_0 <- quartiles_0[1] - 1.5*IQR_0
Upper_0 <- quartiles_0[2] + 1.5*IQR_0
 
data_no_outlier_0 <- subset(survey_final_gis[survey_final_gis$any_adaptation==0,]$vechval, survey_final_gis[survey_final_gis$any_adaptation==0,]$vechval> Lower_0 & survey_final_gis[survey_final_gis$any_adaptation==0,]$vechval< Upper_0)
...
```
As p-value for F test of AONVA is very small, rainfall in three regions is not the same. It can be seen the rainfall in region "3"Rolim de Moura is less than that in the other two.  
...
divide rainfall
```{r}
rainfall_group<- cut(survey_final_gis$rainfall, c(0,1950,2300), label=c("low","high"))
survey_final_gis2 <- mutate(survey_final_gis, rainfall_group)
aov_incbeef2 <- aov(incbeef ~ rainfall_group, data=survey_final_gis2)
summary(aov_incbeef2)
TukeyHSD(aov_incbeef2, conf.level=.95)
```
