...
```{r}
aov_incbeef <- aov(incbeef ~ studycode, data=survey_final_gis_clean,na.rm=TRUE)
summary(aov_incbeef)
TukeyHSD(aov_incbeef, conf.level=.95)
```
As p value is larger than 0.05, we ca not reject our hypothesis that the means of income from beef at different regions are the same.  

Next, we transform rainfall date from wide form to long form.
```{r}
survey_final_gis_clean_season <- survey_final_gis_clean %>% gather(key=season, value=rainfall_seaon, c("rainfall_dry","rainfall_wet"))
