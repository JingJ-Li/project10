...
survey2 <- read_dta("data/merged_survey_panel96_19_noID.dta")
#dim(survey)
dim(survey2)
```
# creating adaptation variables
```{r}
cattle <- c("feed_cattle","soil_analysis","cattleinputs_breed","semiconfine","sellcattle_droughtexp")
Pasture <- c("irrigation_pas","pasture_productivity","fert_pasture","pest_pasture","fallow")
forest <- c("forest_rec","keep_veg")
water_manag <- c("waterstructure", "well_have","dam_have","reservoir_have")
water_accs <- c("trough_have","caixa_seca_have")
adapt <- c(cattle,Pasture,forest,water_manag,water_accs)
#adapt_v <- matrix(NA,nrow=nrow(survey2),ncol=length(survey2)) 
survey2$adapt_v <- vector(length=nrow(survey2))
survey2$adapt_v <-c()
for (i in 1:nrow(survey2)){
  for (j in 1:length(adapt)){
    if (survey2[i,which(colnames(survey2)==adapt[j])]== 1){
      survey2$adapt_v[i] <- 1
      break
    }
    else {
...
>>>>>>> 538973a9eb372bf8904b4e9ff5aa531b7a20a5cc
...
```{r}
t.test(survey2$milkincwet,survey2$milkincdry,na.rm=TRUE,var.equal = FALSE)
```
...
```{r}
t.test(survey2$SPI_wet6,survey2$SPI_dry6,na.rm=TRUE,var.equal = FALSE)
t.test(survey2$SPI_wet,survey2$SPI_dry,na.rm=TRUE,var.equal = FALSE)
```
While there is no significant difference in SPI of average SPI of six months, average monthly SPI in the peak of wet  season is significantly higher than that of dry season.   
## drainage data treatment

Three drainage data will be combined 
```{r}
# first, NA will be converted into 0
ifelse(is.na(survey2$Drainage_AreaKM_Ari), 0, survey2$Drainage_AreaKM_Ari)
ifelse(is.na(survey2$OPO_area_km), 0, survey2$OPO_area_km)
ifelse(is.na(survey2$RO_area_km), 0, survey2$RO_area_km)
drainage_area_km <- rowSums(survey2[,c("Drainage_AreaKM_Ari","OPO_area_km","RO_area_km")],na.rm=TRUE)
mutate(survey2,drainage_area_km) 
...
```{r, include=FALSE}
drop_na(survey2,studycode)
```


```{r}
result <- aov(drainage_area_km ~ studycode, data=survey2)
summary(result)
